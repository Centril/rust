...
fn check_statement(
    ...
    match &statement.kind {
        ...
        if !tcx.features().active(sym::const_if_match)
        => {
...
fn check_place(
    ...
    while let &[ref proj_base @ .., elem] = cursor {
        cursor = proj_base;
        match elem {
            ProjectionElem::Downcast(..) if !tcx.features().active(sym::const_if_match)
...
fn check_terminator(
    ...
    match &terminator.kind {
        ...
        TerminatorKind::Unreachable if tcx.features().active(sym::const_if_match) => Ok(()),
