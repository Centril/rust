...
use rustc_lint_types::LintDirective;
...
/// A mapping from HIR nodes to any lint control directives they might have.
///
/// In lowering, we will avoid inserting to the map if there are no such attributes.
/// However, it would work just as well if we did.
/// But we can use `SmallVec<[_; 1]>` under the assumption that 0 is most common, then 1, ...
pub type LintDirectivesMap = FxIndexMap<HirId, SmallVec<[LintDirective; 1]>>;
...
pub struct Crate<'hir> {
    ...
    /// A mapping from HIR nodes to any lint control directives they might have.
    pub lint_directives: LintDirectivesMap,
