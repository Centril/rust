    ...
    fn perform_lint(&self, cx: &LateContext<'_, '_>, what: &str, id: hir::HirId,
        ...
        match vis.node {
            hir::VisibilityKind::Public if !cx.access_levels.is_reachable(id) => {
                ...
                let replacement = if cx.tcx.features().active(sym::crate_visibility_modifier) {
                    "crate"
                } else {
                    "pub(crate)"
                }.to_owned();
...
impl<'a, 'tcx> LateLintPass<'a, 'tcx> for TrivialConstraints {
    fn check_item(
        ...
        if cx.tcx.features().active(sym::trivial_bounds) {
...
impl<'a, 'tcx> LateLintPass<'a, 'tcx> for ExplicitOutlivesRequirements {
    fn check_item(&mut self, cx: &LateContext<'a, 'tcx>, item: &'tcx hir::Item) {
        ...
        let infer_static = cx.tcx.features().active(sym::infer_static_outlives_requirements);
        let def_id = cx.tcx.hir().local_def_id(item.hir_id);
